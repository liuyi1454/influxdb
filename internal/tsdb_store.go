...
type TSDBStoreMock struct {
	...
	FieldKeysFn               func(auth query.CoarseAuthorizer, sources influxql.Sources) (map[string][]string, error)
	OpenFn                    func() error
	PathFn                    func() string
...
func (s *TSDBStoreMock) FieldKeys(ctx context.Context, auth query.CoarseAuthorizer, sources influxql.Sources) (map[string][]string, error) {
	return s.FieldKeysFn(auth, sources)
}
